<?php class Komentar extends CI_Controller{
var $imagePath = 'public/media/posts/';
	...
	function __construct(){
		...
		$this->load->model('Mkomentar','',TRUE);
	}
	function index(){
		$this->auth->restrict();
		...
		$data['base_url'] = base_url().'admin/komentar/index/';
		$data['total_rows'] = $this->db->count_all('tbl_komentar');
		$data['per_page'] = 10;
		$data['uri_segment'] = 4;
		$data['num_links']= 3;
		$data['next_link']='<';
		...
		$data['komentar']=$this->Mkomentar->getAll($data['per_page'],$this->uri->segment(4,0));
		$data['content']='admin/komentar/data_komentar';
		$this->load->view('admin/template',$data);
	}
	
	function edit_komen($id) {
		$this->auth->restrict();
		/*$this->auth->cek_menu(2);*/
			$level = $this->session->userdata('id_level');
			$data['menu'] = $this->Muser->get_menu_for_level($level);
		$this->form_validation->set_rules('isi_balasan', 'isi_balasan', 'required|xss_clean');

		$this->form_validation->set_error_delimiters('', '<br/>');
		if ($this->form_validation->run() == FALSE) {
			$data['balasan'] = $this->Mkomentar->select($id);
			$data['content']='admin/komentar/balas_komentar';
			$this->load->view('admin/template', $data);
